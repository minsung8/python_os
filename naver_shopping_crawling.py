...
output_path = script_path + "/data_raw"
...
def get_review(keyword, page):
	...
	for i in range(1, page+1):
		...
		html = s.get(naver_shopping_url, params=params).text
		...
		for product in product_list:
			if product['item']['purchaseConditionInfos']:
				if len(product['item']['mallProductUrl']) > 0:
					smart_store_product_url_list.append([product['item']['mallProductId'], product['item']['mallPcUrl'].split('/')[-1],product['item']['productTitle']])
				else:
					naver_shopping_product_id_list.append([product['item']['parentId'],product['item']['productTitle']])
		sleep(1)
	...
	for url in tqdm(smart_store_product_url_list):
		try:
	...
	for id in tqdm(naver_shopping_product_id_list):
		for i in range(1, 101):
			try:
				res = r2.request_review(id[0], i)
				data = json.loads(res)
				for review in data['reviews']:
					# print(review)
					try:
						temp = {}
						temp['productname'] = id[1]
						# print(id[1])
						temp['productNo'] = id[0]
	...
	wr = csv.writer(f)
	wr.writerow([1, 'productNo', 'createDate', 'writerMemberId', 'productname','id', 'reviewContent', 'reviewScore'])
	for i in range(len(review_list)):
		wr.writerow([i + 2, review_list[i]['productNo'], review_list[i]['createDate'], review_list[i]['writerMemberId'],
					 review_list[i]['productname'], review_list[i]['id'],
					 review_list[i]['reviewContent'], review_list[i]['reviewScore']])
	f.close()
	return
...
if __name__ == '__main__':
    ...
    get_review('책상', 1)
	# if len(sys.argv) < 2:
	# 	print("Usage: %s keyword page_num\n" % sys.argv[0])
	# 	print("ex) %s 책상 1" % sys.argv[0])
	# 	sys.exit(1)
	...
	# output_file = "%s/review_%s_%d_%s.csv" % (output_path, keyword, page, today)

	# # check path
	# if not os.path.exists(output_path):
	# 	print("* Output path [%s] is not found." % output_path)
	# 	sys.exit(1)
